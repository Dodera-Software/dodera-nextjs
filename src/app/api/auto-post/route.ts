...
import { db } from "@/db";
import { autoGeneratedBlogPosts } from "@/db/schema";
import {
    SYSTEM_PROMPT,
    getRandomTopicHint,
    parseGeneratedPost,
...
export async function GET(request: NextRequest) {
    ...
    try {
        ...
        if (recentPosts.length > 0) {
            ...
            console.log("[auto-post] No previous posts found in the database — first generation.");
        }
    } catch (dbErr) {
    ...
    try {
        ...
        /* ── 7b. Record post for future deduplication ────────────── */
        try {
            ...
            console.log(`[auto-post] Post recorded in the database — uid="${generatedPost.uid}"`);
        } catch (insertErr) {
            console.warn("[auto-post] Dedup-record insert failed (post saved to Prismic, deduplication record skipped):", insertErr);
        }
