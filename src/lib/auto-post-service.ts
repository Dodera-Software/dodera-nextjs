...
import { db } from "@/db";
import { autoGeneratedBlogPosts, blogPostExamples } from "@/db/schema";
...
export async function autoPost(options: AutoPostOptions = {}): Promise<AutoPostServiceResult> {
    ...
    /* 1b. Fetch recent posts for topic deduplication */
    let recentPostsContext = "";
    try {
        const recentPosts = await db
            .select({
                title: autoGeneratedBlogPosts.title,
                category: autoGeneratedBlogPosts.category,
                uid: autoGeneratedBlogPosts.uid,
            })
            .from(autoGeneratedBlogPosts)
            .orderBy(desc(autoGeneratedBlogPosts.createdAt))
            .limit(20);
        ...
        if (recentPosts.length > 0) {
            ...
            console.log("[auto-post-service] No previous posts in the database — first generation.");
        }
    } catch (dbErr) {
    ...
    try {
        const exampleRows = await db
            .select({ content: blogPostExamples.content })
    ...
    try {
        ...
        /* Record in the database for future deduplication */
        try {
            await db.insert(autoGeneratedBlogPosts).values({
                uid: generatedPost.uid,
                title: generatedPost.title,
                category: generatedPost.category,
                tags: generatedPost.tags,
            });
            console.log(`[auto-post-service] Post recorded in the database — uid="${generatedPost.uid}"`);
        } catch (insertErr) {
            console.warn("[auto-post-service] Dedup-record insert failed — non-critical:", insertErr);
        }
